...
static CONSTANTS: LazyLock<RwLock<HashMap<&'static str, Value>>> = LazyLock::new(|| {
...
static BUILT_IN_FUNCTIONS: LazyLock<RwLock<Vec<Function>>> = LazyLock::new(|| {
    let m = vec![
...
pub fn remove_built_in_function(func_identifier: &str) -> Option<Function> {
    BUILT_IN_FUNCTIONS
        ...
        .map(|index| BUILT_IN_FUNCTIONS.write().unwrap().swap_remove(index))
}
